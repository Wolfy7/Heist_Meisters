...
onready var light = $VSplitContainer/ButtomContainer/ButtonGrid/Light

var combination_correct_sfx ="res://SFX/threeTone1.ogg"
var button_pressed_sfx = "res://SFX/twoTone1.ogg"
...
func connect_buttons():
	for child in $VSplitContainer/ButtomContainer/ButtonGrid.get_children():
		if child is Button:
...
func check_guess():
	if guess == combination:
		$AudioStreamPlayer.stream = load(combination_correct_sfx)
...
func enter(button):
	$AudioStreamPlayer.stream = load(button_pressed_sfx)
	$AudioStreamPlayer.play()
	guess.append(button)
	update_display()
